...
namespace TestProject1
{
    public class Tests
    {
        ...
        #region get file stream from archive
        [Test]
        [TestCase(@"D:\VS projects\ArchivingServices\TestProject1\bin\Debug\net5.0\NewFolder.zip", "1.jpg")]
        public void get_file_stream_from_archive(string ArchiveFilePath, string fileName)
        {
            ZipArchive _zipFile = ZipFile.Open(ArchiveFilePath, ZipArchiveMode.Update);
            var expected = ReturnSingleFileStream(_zipFile, fileName);
        }
        #endregion

        #region metadate
        [Test]
        [TestCase(@"D:\VS projects\ArchivingServices\TestProject1\bin\Debug\net5.0\NewFolder2.zip")]
        public void GetFilesMetadataFromArchive_Working_with_file_contains_one_subfile(string filePath)
        {
            ...
            {
                Assert.NotNull(file.CompressedLength);
        ...
        [Test]
        [TestCase(@"D:\VS projects\ArchivingServices\TestProject1\bin\Debug\net5.0\NewFolder.zip")]
        public void GetFilesMetadataFromArchive_Working_with_file_contains_multiple_subfiles(string filePath)
        {
            ZipArchive _zipFile = ZipFile.Open(filePath, ZipArchiveMode.Read);
            foreach (var file in ArchivingServicess.GetFilesMetadataFromArchive(filePath))
            {
        ...
        [Test]
        [TestCase(@"D:\VS projects\ArchivingServices\TestProject1\bin\net5.0\NewFolder.zip")]
        public void GetFilesMetadataFromArchive_Not_Working_with_invalid_path(string filePath)
        {
            ...
            Assert.That(ex.Message, Is.EqualTo(@"Could not find a part of the path 'D:\VS projects\ArchivingServices\TestProject1\bin\net5.0\NewFolder.zip'."));
        }
        [Test]
        [TestCase(null)]
        public void GetFilesMetadataFromArchive_Not_Working_with_null_path(string filePath)
        {
            ...
            Assert.That(ex.Message, Is.EqualTo("Value cannot be null. (Parameter 'Archive Files Paths can't be null')"));
